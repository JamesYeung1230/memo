...
2. [.trae/rules/project_rules.md](../../.trae/rules/project_rules.md) — 开发规则锚点
...
以下规则是对上述项目规则的补充，聚焦 AI Agent 在执行 Web 管理端前端开发任务时的行为约束。
...
- 每次任务开始前必须加载以下锚点文档：
  1. [project.md](../../project.md) — 项目总览与当前阶段
  2. [本文件](project_rules.md) — 项目开发规则锚点
...
- 禁止引入项目未使用的新依赖，除非用户明确要求
- 使用新依赖前，先检查 `package.json` 中「不推荐的技术」列表，确认不是被禁止的库
