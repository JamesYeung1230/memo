...
> AI Agent 在执行任务时自动遵守 Part B 和 Part C 的规则；Part A 在任务完成后的提交阶段生效。
...
| 文件位置 | 职责 | 包含板块 |
|---------|------|---------|
| `.trae/rules/project_rules.md`（根） | 项目全局规则 | Part A（Git）、Part B（AI 通用）、Part C（Skill）、Part D（维护指南）、附录 |
...
1. **根规则统管全局**：Part A（Git 工作流）和 Part C（Skill 管理）全局适用，子项目不重复定义
